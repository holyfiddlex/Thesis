...
Generative models suffer an even greater overhead, which is a great critique to models like WaveNet which take a point by point generation approach. *** % Cite WaveNet
\par
...
\qquad In deep learning, data wrangling, also known as data munging, consists of changing the data in a way that is convenient to be processed by the model. *** % Link to appendix and reference https://www.xenonstack.com/blog/data-preparation/
...
To do this, you can use a resampling algorithm, the one most recorded in most public repositories is a poly-phase resampling algorithm. *** % Place in appendix
The algorithm is not difficult to implement but can also be found in certain libraries that can be used for signal processing, like scipy. *** % Link to scipy
\par
In python, it is somewhat accepted that the normal resampling function that comes with the common signal processing libraries is not good enough. *** % https://signalsprocessed.blogspot.com/2016/08/audio-resampling-in-python.html
Instead, polyphase sampling is preferred due to it's efficiency and cleaner results.
Although there are still certain problems found in the algorithm found in python libraries, it still presents a great improvement without having to climb down the deep rabbit hole that is up and down sampling theory. *** % blog where there was a critique http://signalsprocessed.blogspot.com/2017/05/resampling-in-python-electric-bugaloo.html
% also a link to the official documentation saying that poly is faster than resample https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.resample_poly.html
\par
...
Using this method, if the new sample rate is a divisor of the old we have the exact same result that was mentioned before. *** % Pointer to third paragraph of subsection.
...
\par
Adding zeroes also allows for data leakage to occur, which could reduce our performance outside of training.  %Data Leakage in appendix
...
Certain studies even go as far as to transform spectrograms to rgb channeled images to directly use with well known images classification techniques. *** % Get an example of this
\par
